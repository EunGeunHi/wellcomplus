...
// 납품서용 공지사항 조회 (관리자 권한 필요)
export const GET = withKingAuthAPI(async (request, { session }) => {
  try {
    await connectDB();

    // 납품서용 공지사항 조회 (최신 문서 하나만)
    const announcement = await QuoteAnnouncement.findOne({ type: 'delivery' })
      .sort({ updatedAt: -1 })
      .exec();
...
export const POST = withKingAuthAPI(async (request, { session }) => {
  try {
    ...
    // 최신 납품서용 공지사항 조회
    let existingAnnouncement = await QuoteAnnouncement.findOne({ type: 'delivery' })
      .sort({ updatedAt: -1 })
      .exec();
