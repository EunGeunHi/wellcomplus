...
export default function DeliveryQuotePage({ params }) {
  ...
  useEffect(() => {
    const fetchAnnouncement = async () => {
      try {
        // API를 통해 공지사항 데이터 불러오기 (납품서용 공지사항 API 호출)
        ...
        // 기본 공지사항 설정
        setNoticeItems([
          '본 납품서는 수급상황에 따라, 금액과 부품이 대체/변동 될 수 있습니다.',
  ...
  const saveNotesContent = async () => {
    ...
    try {
      ...
      // API를 통해 공지사항 저장 (납품서용 공지사항 API 호출)
      const response = await fetch('/api/quote/delivery', {
        method: 'POST',
        headers: {
            ...
            <table className="w-full border-collapse border border-sky-200">
              <tbody>
                <tr className="bg-sky-50">
                  <td className="border border-sky-200 p-2 w-1/2 font-semibold text-black text-center">
                    담당자
                  </td>
                  <td className="border border-sky-200 p-2 w-1/2 font-semibold text-black text-center">
                    공급자
                ...
                <tr>
                  <td className="border border-sky-200 p-1">
                    납품 장소: {estimate.customerInfo?.name || ''}
                  </td>
                  <td className="border border-sky-200 p-1">공급자: 웰컴시스템</td>
                </tr>
                <tr>
                  <td className="border border-sky-200 p-1">납품주소: </td>
                  <td className="border border-sky-200 p-1">
                    주 &nbsp;소: 부산시 동래구 온천장로 20 부산컴퓨터도매상가 209호
                  </td>
                </tr>
                <tr>
                  <td className="border border-sky-200 p-1">
                    연락처: {estimate.customerInfo?.phone || ''}
                  </td>
                  <td className="border border-sky-200 p-1">연락처: 051-926-6604, 010-8781-8871</td>
                </tr>
                <tr>
                  <td className="border border-sky-200 p-1">
                    <div className="flex justify-between items-center">
                      <span>담당확인자:</span>
                      <span className="text-xs text-gray-500 mr-12">(인)</span>
                    </div>
                  </td>
                  <td className="border border-sky-200 p-1">
                    <div className="flex justify-between items-center">
                      ...
                      <span className="text-xs text-gray-500 mr-12 relative">
                        (인)
                        <span className="relative inline-block">
                          {showStamp && (
                            <div
